...
gcode:
    ...
    // 3. LIGHTING (DIMMABLE)
    // idle_params and print_params contain strings like "RED=0.5 GREEN=0 BLUE=0"
    lighting: (name, idle_params, print_params) => 
`#--------------------------------------------------------------------
# INDIVIDUAL COLOR MACROS (Standard Brightness for Cycle)
#--------------------------------------------------------------------
[gcode_macro LED_RED]
gcode: SET_LED LED=${name} RED=1.0 GREEN=0.0 BLUE=0.0 TRANSMIT=1

[gcode_macro LED_ORANGE]
gcode: SET_LED LED=${name} RED=1.0 GREEN=0.5 BLUE=0.0 TRANSMIT=1

[gcode_macro LED_YELLOW]
gcode: SET_LED LED=${name} RED=1.0 GREEN=1.0 BLUE=0.0 TRANSMIT=1

[gcode_macro LED_GREEN]
gcode: SET_LED LED=${name} RED=0.0 GREEN=1.0 BLUE=0.0 TRANSMIT=1

[gcode_macro LED_TEAL]
gcode: SET_LED LED=${name} RED=0.0 GREEN=0.5 BLUE=1.0 TRANSMIT=1

[gcode_macro LED_BLUE]
gcode: SET_LED LED=${name} RED=0.0 GREEN=0.0 BLUE=1.0 TRANSMIT=1

[gcode_macro LED_PURPLE]
gcode: SET_LED LED=${name} RED=1.0 GREEN=0.0 BLUE=1.0 TRANSMIT=1

[gcode_macro LED_WHITE]
gcode: SET_LED LED=${name} RED=0.8 GREEN=0.8 BLUE=0.8 TRANSMIT=1

[gcode_macro LED_OFF]
gcode: SET_LED LED=${name} RED=0.0 GREEN=0.0 BLUE=0.0 TRANSMIT=1

#--------------------------------------------------------------------
# STATUS MACROS (Custom Brightness)
#--------------------------------------------------------------------
